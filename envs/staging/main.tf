...
locals {
   ...
   * Base network ranges
   */
  base_private_service_cidr = "10.41.0.0/24"
  base_subnet_primary_ranges = {
    (local.default_region1) = "10.21.0.0/22"
  ...
  base_subnet_secondary_ranges = {
    (local.default_region1) = [
        
       {
         ...
         ip_cidr_range = "10.23.0.0/22"
       },
        {
         range_name    = "sb-${module.base_env.base_network_name}-${local.default_region1}-gke-pod-01"
         ip_cidr_range = "10.24.0.0/16"
       },
      {
        range_name    = "sb-${module.base_env.base_network_name}-${local.default_region1}-gke-svc"
        ip_cidr_range = "10.22.0.0/22"
      }
    ]
    # (local.default_region2) = [
    #    {
    #      range_name    = "sb-${local.environment_code}-shared-base-${local.default_region2}-gke-pod"
    #      ip_cidr_range = "192.168.16.0/20"
    #    },
    #   {
    #     range_name    = "sb-${local.environment_code}-shared-base-${local.default_region2}-gke-svc"
...
#   name          = "sec-core-${local.environment_code}-org-prj-shared-base-us-east1"  
